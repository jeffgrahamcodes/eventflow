...
PAYMENT_TABLE = os.environ.get(
    "PAYMENT_RECORDS_TABLE_NAME", "eventflow-payment-records"
)
...
def wait_for_record(table_name: str, key: dict, description: str) -> dict:
    ...
    raise AssertionError(
        f"✗ {description} not found in {table_name} after {MAX_RETRIES * WAIT_SECONDS}s"
    )
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        response = (
            table.query(
                ...
                KeyConditionExpression=boto3.dynamodb.conditions.Key("order_id").eq(
                    order_id
                ),
            )
            if False
        ...
        time.sleep(WAIT_SECONDS)
    raise AssertionError(
        f"✗ Payment record not found for order_id: {order_id} "
        f"after {MAX_RETRIES * WAIT_SECONDS}s"
    )
